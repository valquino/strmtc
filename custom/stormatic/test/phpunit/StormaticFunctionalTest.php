...
/**
 * \file    test/phpunit/StormaticFunctionalTest.php
 * \ingroup stormatic
 ...
 * @package Test
 */
class StormaticFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modStormatic')->click();
		} else {
			// Disable the module
			$this->byHref('modStormatic')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/stormatic/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('stormatic/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/stormatic/admin/about.php');
		$this->authenticate();
		return $this->assertContains('stormatic/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('widget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modStormatic_StormaticTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modStormatic_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
