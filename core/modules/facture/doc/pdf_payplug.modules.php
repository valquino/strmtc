...
/**
 *	\file       htdocs/core/modules/facture/doc/pdf_payplug.modules.php
 *	\ingroup    facture
 *	\brief      File of class to generate customers invoices from payplug model
 */
...
/**
 *	Class to manage PDF invoice template Payplug
 */
class pdf_payplug extends ModelePDFFactures
{
    var $db;
    var $name;
    var $description;
	...
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "payplug";
		$this->description = $langs->trans('PDFPayplugDescription');
	...
	function _tableau_info(&$pdf, $object, $posy, $outputlangs)
	{
		...
		if ($object->type != 2)
		{
			...
			// Affichage du lien Payplug - Start {
			if (empty($object->mode_reglement_code) || $object->mode_reglement_code == 'CB')
			{
				if(file_exists(DOL_DOCUMENT_ROOT.'/payplug/lib/payplug.lib.php'))
				{
					require_once DOL_DOCUMENT_ROOT.'/payplug/lib/payplug.lib.php';
		            ...
		            // Save payment URL in extra field
		            $DocumentType = $object->element == 'propal' ? 1 : 2;
		            $PaymentURL = GetPaymentURL($DocumentType, $object->id);
	            	
	            	if(isset($object->array_options[PAYPLUG_EXTRAFIELD_PAYMENTLINK]) && $object->array_options[PAYPLUG_EXTRAFIELD_PAYMENTLINK] != '')
	            	{
	            		$diffsizetitle=(empty($conf->global->PDF_DIFFSIZE_TITLE)?3:$conf->global->PDF_DIFFSIZE_TITLE);
	            		
						$pdf->SetFont('','B', $default_font_size - $diffsizetitle);
						$pdf->SetXY($this->marge_gauche, $posy-2);
						$pdf->MultiCell(100, 1, 'Paiement par carte:',0,'L');
						
						$pdf->SetTextColor(0,0,255);
						$pdf->SetFont('','U', $default_font_size - $diffsizetitle);
						$pdf->SetXY($posxval, $posy-2);
						$pdf->MultiCell(40, 1, 'Cliquez ici pour payer en ligne',0,'L');
						$pdf->Link($posxval,$posy-2, 40, 4, $object->array_options[PAYPLUG_EXTRAFIELD_PAYMENTLINK]);
						$pdf->SetTextColor(0,0,0);
						
						// Affichage du QRCode
						$QRCodeURL = 'https://api.qrserver.com/v1/create-qr-code/?size=150x150&data='.$object->array_options[PAYPLUG_EXTRAFIELD_PAYMENTLINK];
						$pdf->Image($QRCodeURL, $posxval+50,$posy-6, 15,15,'PNG');
						$pdf->Link($posxval+50,$posy-6, 15,15, $object->array_options[PAYPLUG_EXTRAFIELD_PAYMENTLINK]);
						
						$posy=$pdf->GetY()+2;
	            	}
				}
			}
			// Affichage du lien Payplug - End }
